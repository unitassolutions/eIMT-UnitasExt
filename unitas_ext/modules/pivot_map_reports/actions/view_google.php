...
/* ---------------- ZOOM ---------------- */
if($use_form && strlen($reports['zoom']))
{
...
if($use_form && strlen(trim($reports['latlng'])))
{
...
{
    $center_map_js = "
        var myLatlng = new google.maps.LatLng(" . (float)$map_cfg['default_lat'] . "," . (float)$map_cfg['default_lng'] . ");
        map.setCenter(myLatlng);
        map.setZoom($zoom);
    ";
}
