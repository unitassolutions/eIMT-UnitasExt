...
$pivot_map_info_query = db_query("select * from app_unitas_pivot_map_reports where id='" . _get::int('reports_id') . "'");
...
switch($app_module_action)
{
    case 'save':
        ...
        if(isset($_GET['id']))
        {
            db_perform('app_unitas_pivot_map_reports_entities', $sql_data, 'update', "id='" . db_input($_GET['id']) . "'");
            $pivot_map_id = $_GET['id'];
        }
        else
        {
            db_perform('app_unitas_pivot_map_reports_entities', $sql_data);
        ...
        redirect_to('unitas_ext/pivot_map_reports/entities', 'reports_id=' . $pivot_map_info['id']);

        break;
    case 'delete':

        db_delete_row('app_unitas_pivot_map_reports_entities', _get::int('id'));
    ...
    case 'get_entities_fields':
        ...
        if(isset($_POST['id']))
        {
            $obj = db_find('app_unitas_pivot_map_reports_entities', $_POST['id']);
        }
        else
        {
            $obj = db_show_columns('app_unitas_pivot_map_reports_entities');
        }
