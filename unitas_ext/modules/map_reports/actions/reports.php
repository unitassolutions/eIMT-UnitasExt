...
switch ($app_module_action)
{
    case 'save':
        ...
        $sql_data = array(
            ...
            'use_form_settings' => $_POST['use_form_settings'] ?? 0,
        );

        if (isset($_GET['id']))
        {
            db_perform('app_unitas_map_reports', $sql_data, 'update', "id='" . db_input($_GET['id']) . "'");
        }
        else
        {
            db_perform('app_unitas_map_reports', $sql_data);
        }

        redirect_to('unitas_ext/map_reports/reports');
    ...
    case 'delete':
        ...
        db_delete_row('app_unitas_map_reports', $_GET['id']);
    ...
    case 'get_entities_fields':
        ...
        if (isset($_POST['id']))
        {
            $obj = db_find('app_unitas_map_reports', $_POST['id']);
        }
        else
        {
        ...
        $choices = array();
        $fields_query = db_query("select * from app_fields where type in ('fieldtype_mapbbcode','fieldtype_yandex_map','fieldtype_google_map','fieldtype_google_map_directions','fieldtype_unitas_geometry','fieldtype_unitas_location') and entities_id='" . db_input($entities_id) . "'");
